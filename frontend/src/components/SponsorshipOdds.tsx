...
import { Briefcase, Clock, TrendingUp, CheckCircle2 } from 'lucide-react';
...
function SponsorshipOdds({ companyId }: SponsorshipOddsProps) {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        {!companyId && (
          <div>
            ...
            {!watchedCompanyId && (
              <p className="mt-2 text-xs text-text-muted">Visit a company page to get their ID, or use the salary predictor above.</p>
            )}
          </div>
        )}
      ...
      {mutation.data && (
        <div className="mt-8 border-t border-white/5 pt-8">
          <div className="text-center mb-6">
            <div className={`text-4xl sm:text-5xl font-black tracking-[-0.05em] font-display ${getLikelihoodColor(likelihoodPercentage)}`}>
          ...
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="p-4 border border-white/5 rounded-2xl">
              <div className="text-xs uppercase tracking-wider text-text-muted mb-1">Similar Job Approvals</div>
              <div className="text-lg font-black tracking-[-0.04em] text-text-primary">{mutation.data.likelihood.similar_job_approvals}</div>
            </div>
            <div className="p-4 border border-white/5 rounded-2xl">
              <div className="text-xs uppercase tracking-wider text-text-muted mb-1">Historical Applications</div>
              <div className="text-lg font-black tracking-[-0.04em] text-text-primary">{mutation.data.details.total_historical_apps}</div>
            </div>
          </div>
