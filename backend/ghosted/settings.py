...
PAGINATION_COUNT_CACHE_TIMEOUT = int(
    os.getenv("PAGINATION_COUNT_CACHE_TIMEOUT", str(CACHE_TTL_SECONDS))
)
...
RESUME_SESSION_STORAGE_DIR = Path(
    os.getenv(
        "RESUME_SESSION_STORAGE_DIR",
        str(Path(gettempdir()) / "ghosted_resume_sessions"),
    )
)
...
# Rate limiting (requires django-ratelimit or similar in production)
